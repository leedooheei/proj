...
class AdminFunctionWindow(QMainWindow):
    def __init__(self, username=None, currentPW=None, parent=None):
        ...
        print("Received username:", self.username) #나중에 지워야함
...
class AddMenuWindow(QDialog):
    ...
    def add_menu(self):
        ...
        try:
            ...
            values = (
            self.username, menu_name, float(menu_price), menu_category, *selected_details, *[None] * (5 - len(selected_details)))
    ...
    def populate_details(self):
        try:
            cursor = self.db.cursor()
            # SQL 쿼리를 수정하여 특정 username에 대한 디테일을 선택합니다.
